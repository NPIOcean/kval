...
def rotate_uv(u: np.ndarray, v: np.ndarray, angle: float, 
              in_degrees: bool = False) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def principal_angle(
    ...
    [-π/2, π/2], where the squared distances to `u` and `v` are maximized. 
    The angle is calculated using the method described by Emery and Thompson 
    ...
    in_degrees : bool, optional
        If True, the principal angle is returned in degrees. Default is False (radians).
    return_std : bool, optional
        If True, also return the standard deviation along the major and minor axes.
        Default is False (only return the principal angle).

    Returns:
    -------
    Tuple[Union[float, float], Optional[float], Optional[float]]
        - The principal angle, in radians by default or degrees if `in_degrees` is True.
          Provided in the [-pi/2, pi/2] or [-180, 180] range.
        - Standard deviation along the major axis if `return_std` is True, otherwise None.
        - Standard deviation along the minor axis if `return_std` is True, otherwise None.
    ...
    principal_angle = np.degrees(principal_angle_rad) if in_degrees else principal_angle_rad
